...
describe OrganizationsController do
  ...
  describe "GET search" do
    ...
    context 'setting appropriate view vars for all combinations of input' do
      ...
      pending "sets up appropriate values for view vars: query_term, organizations and json" do
      ...
      pending "handles lack of category gracefully" do
        Organization.should_receive(:search_by_keyword).with('test').and_return(result)
        result.should_receive(:filter_by_category).with(nil).and_return(result)
        get :search, :q => 'test'
      end

      pending "handles lack of query term gracefully" do
      ...
      pending "handles lack of id gracefully" do
        Organization.should_receive(:search_by_keyword).with('test').and_return(result)
        result.should_receive(:filter_by_category).with(nil).and_return(result)
      ...
      pending "handles empty string id gracefully" do
        Organization.should_receive(:search_by_keyword).with('test').and_return(result)
        result.should_receive(:filter_by_category).with(nil).and_return(result)
    ...
    pending "assigns to flash.now but not flash when search returns no results" do
    ...
    pending "does not set up flash nor flash.now when search returns results" do
      result = [double_organization]
      json='my markers'
      result.should_receive(:to_gmaps4rails).and_return(json)
  ...
  describe "GET index" do
    pending "assigns all organizations as @organizations" do
      result = [double_organization]
      json='my markers'
      result.should_receive(:to_gmaps4rails).and_return(json)
